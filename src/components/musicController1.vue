...
const currentSong = ref(musicService.getCurrentSong())
...
const handleSongChange = () => {
  currentSong.value = musicService.getCurrentSong()
}
...
onMounted(() => {
  // 同步当前状态
  currentTime.value = Number(musicService.getCurrentTime())
  duration.value = Number(musicService.getDuration())
  isPlaying.value = musicService.getIsPlaying()
  volume.value = Number(musicService.getVolume())
  
  // 监听事件
  musicService.on('timeupdate', handleTimeUpdate)
  musicService.on('durationupdate', handleDurationUpdate)
  musicService.on('play', handlePlay)
  musicService.on('pause', handlePause)
  musicService.on('volumechange', handleVolumeChange)
  musicService.on('songchange', handleSongChange)
